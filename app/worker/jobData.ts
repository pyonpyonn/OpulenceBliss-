...
export async function loadWorkerJob(
  ...
  const { data: row } = await supabase
    .from("bookings")
    .select(
      "id, created_at, scheduled_at, status, address, household_notes, customer_id, customer_email, provider_id, provider_payout, subscription_id, packages(name, duration_minutes), check_ins(arrived_at, left_at, geofence_pass, gps_lat, gps_lng)",
    )
    .eq("id", bookingId)
    .maybeSingle();
