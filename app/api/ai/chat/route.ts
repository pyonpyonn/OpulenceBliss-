...
const CLIENT_TOOLS = [
  ...
  },
  {
    ...
    description:
      "Prepare a clear Review and pay button once the customer has chosen a service, postcode and exact available time. Card payment always requires the customer to finish on the secure checkout page.",
    parameters: {
      type: "OBJECT",
      properties: {
        service_name: {
  ...
  },
  {
    ...
    description:
      "Check the real reschedule window and list currently available replacement times for one booking. Use before preparing a reschedule.",
    parameters: {
      type: "OBJECT",
      properties: {
        booking_id: {
...
async function ownClientBooking(context: AgentContext, bookingId: string) {
  ...
  const { data, error } = await context.client
    .from("bookings")
    .select(
      "id, customer_id, status, scheduled_at, address, package_id, packages(name, service_type)",
    )
    .eq("id", bookingId)
  ...
  if (action.type === "reschedule_booking") {
    ...
    if (!stillAvailable) {
      return {
        ok: false as const,
        message:
          "That time is no longer available. Ask me to find fresh reschedule options.",
      };
    }
    ...
    if (name === "prepare_reschedule_booking") {
      ...
      if (
        ...
        return {
          ok: false,
          error:
            "That time is not in the current availability. Call reschedule_options again.",
        };
      }
      const reason =
...
- For a new booking, gather service, postcode and time, use live availability, then call prepare_booking. The customer must review and pay on the secure page; never say the booking is complete before that.
...
- For rescheduling, call reschedule_options, let the customer choose an exact returned time, then call prepare_reschedule_booking. The 48-hour rule and live availability are enforced again at confirmation.
